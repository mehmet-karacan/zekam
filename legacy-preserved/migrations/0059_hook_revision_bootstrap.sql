-- Preserve immutable hook history while allowing a fresh realm to observe a later revision first.
...
begin
  ...
  if not exists(select 1 from hooks.spec_revision prior
    ...
    and exists(select 1 from hooks.spec_revision prior
      where prior.realm_id=new.realm_id and prior.hook_id=new.hook_id)
