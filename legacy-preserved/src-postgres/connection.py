...
@dataclass(frozen=True, slots=True)
class PostgresRealmSessionOperations:
    """PostgreSQL adapter for the application realm-session boundary."""

    role: str = APPLICATION_ROLE

    def configure(self, connection: Any, *, realm_id: UUID | None = None) -> None:
        configure_session(connection, realm_id=realm_id, role=self.role)
