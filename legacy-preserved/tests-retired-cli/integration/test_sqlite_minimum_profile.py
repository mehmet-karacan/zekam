...
def test_cli_init_to_project_work_and_separate_knowledge_gate(home_root: Path) -> None:
    ...
    assert indexed.exit_code == 70
    assert "operational DB'den ayridir" in indexed.stderr
    ...
    assert projects.exit_code == works.exit_code == 0
    assert searched.exit_code == 70
    assert "operational DB'den ayridir" in searched.stderr
