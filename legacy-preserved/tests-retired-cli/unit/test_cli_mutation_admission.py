...
def test_gate_a_source_capability_cannot_be_issued_outside_exact_cli_leaf() -> None:
    command = ("continuity", "source-bind")
    with pytest.raises(PolicyViolation):
        _issue_gate_a_source_capability(command, confirmed=True)
    with pytest.raises(PolicyViolation):
        _issue_gate_a_source_capability(command, confirmed=False)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("backup", "verify"),
        ("client", "hook"),
        ("local-runtime", "outbox-once"),
        ("local-runtime", "resolve"),
        ("local-runtime", "submit-journal"),
        ("local-runtime", "worker-once"),
        ("opencode", "event"),
...
def test_realm_session_routes_every_unexempted_mutation_through_common_gate(
    ...
    application_context = SimpleNamespace(
        ...
        settings=SimpleNamespace(
            database=SimpleNamespace(backend=PersistenceBackend.SQLITE),
    ...
    monkeypatch.setattr(cli_session, "PostgresRealmSessionOperations", lambda: object())
...
def test_unrelated_single_active_run_cannot_satisfy_exact_target_hint() -> None:
    rows: list[tuple[object, ...]] = [(PROJECT_ID, WORK_ITEM_ID, RUN_ID, "session-one", "codex")]
    admission = DEFAULT_CLI_MUTATION_ADMISSION_REGISTRY.classify(
        ("trace", "start"), {"apply": True, "run_id": RUN_ID_TWO}
    )
    connection = _TargetFilteringConnection(rows, target_run_id=RUN_ID_TWO)
