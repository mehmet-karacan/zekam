...
def _resolve_project_id(realm_context: RealmContext, query: str) -> UUID:
    ...
    return cast(UUID, resolution.resolved.project_id)
...
def _apply_index(
    ...
    *,
    home: str | None,
    embedding: VerifiedLocalEmbeddingBinding,
) -> ProjectIndexResult:
    ...
    result = apply_project_index(
        ...
        knowledge=KnowledgeRepository(
            realm_context.connection, realm_context.realm_id, project_id=plan.project_id
        ),
        retrieval=RetrievalRepository(realm_context.connection, realm_context.realm_id),
    ...
    detail["knowledge_index"] = {
        ...
        "provider_probe_evidence_digest": embedding.profile.probe_evidence_digest,
        "provider_identity_digest": embedding.profile.provider_identity_digest,
    ...
    try:
        ...
        if operational is not None:
            ...
            with operational.unit_of_work() as uow:
                sqlite_project = uow.create_project(
                    slug=selected_slug,
                    display_name=name or resolved.name,
                )
                for item in alias or ():
                    uow.add_project_alias(project_id=sqlite_project.id, alias=item)
                uow.bind_source(
                    project_id=sqlite_project.id,
                    portable_ref=f"source:{selected_slug}",
                    source_kind="git" if kind == "git-repository" else "directory",
                )
                uow.commit()
        ...
        if operational is not None:
            with operational.unit_of_work() as uow:
                rows: list[dict[str, Any]] = [
                    ...
                    for project in uow.list_projects(include_archived=include_archived)
                ]
                uow.commit()
        else:
