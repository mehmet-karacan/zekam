...
from zekam.infrastructure.postgres.legacy_repository_provider import (
    PostgresLegacyRepositoryProvider,
)
...
install_legacy_repository_provider(PostgresLegacyRepositoryProvider())
            ...
            if context.settings.database.backend is PersistenceBackend.POSTGRESQL:
                with connect(context.settings.database) as connection:
                    selected_plan = build_doctor_repair_plan(
                        ...
                        migration_status_reader=migrations.status,
                        routine_status_reader=routine_integrity.status,
                    )
            else:
...
def init(
    ...
    persistence: Annotated[
        ...
        typer.Option(
            "--persistence",
            "--veritabani",
            help="Fresh bootstrap motoru: sqlite; postgresql yalniz legacy detection icindir",
        ),
    ] = None,
) -> None:
    ...
    try:
        ...
        if persistence is PersistenceBackend.POSTGRESQL:
            raise PolicyViolation("PostgreSQL fresh bootstrap veri kaynagi veya runtime olamaz")
        context = build_context(home=home)
