...
def _service(
    ...
    realm, connection = realm_session
    return ModelHealthService(connection, realm, probe=probe or StubProviderProbe())
...
def test_policy_change_makes_the_result_stale(
    ...
    realm, connection = realm_session
    model_id = imported.list_all()[0].model_id
    ModelHealthService(connection, realm, probe=StubProviderProbe()).run_probe(model_id)

    changed = ModelHealthService(
        connection,
        realm,
