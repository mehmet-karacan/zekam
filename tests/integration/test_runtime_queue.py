...
def test_failed_receipt_reconciles_distinct_terminal_recovery_diagnostic(
    host: ExecutionHost,
    realm_session: tuple[Realm, Any],
    project_id: Any,
) -> None:
    ...
    assert host.finish(
        work,
        outcome=AttemptOutcome.RECOVERY_REQUIRED,
        result_digest=diagnostic_digest,
        now=RECOVERY_NOW,
    )
    ...
    assert not finalized.created
    assert host.jobs.get(work.job.id).state is JobState.FAILED
    assert not host.finalize_reconciled_failure(
