...
def _effective_user_id() -> int:
    getter = getattr(os, "getuid")  # noqa: B009 -- POSIX-only branch
    return int(getter())
...
def _read_all(descriptor: int, size: int) -> bytes:
    ...
    while offset < size:
        ...
        chunk = pread(descriptor, min(1024 * 1024, size - offset), offset)
        if not chunk:
...
class SQLiteOperationalBackup:
        ...
        *,
        ...
        if os.name == "nt":
            if destination_parent_descriptor is not None:
                raise ConfigurationError("SQLite backup descriptor anchor unsupported on Windows")
            return self._copy_verified_windows(
                source,
                destination,
                restoring=restoring,
    ...
    def _copy_verified_windows(
        ...
        *,
        ...
        destination.parent.mkdir(parents=True, exist_ok=True)
        if not private_regular(source) or not private_directory(destination.parent):
            raise ConfigurationError("SQLite backup Windows path ACL unsafe")
        ...
        for component in destination.parent.parts[1:]:
            ...
            if not stat.S_ISDIR(info.st_mode) or current.is_symlink() or reparse:
                raise ConfigurationError("SQLite backup destination parent unsafe")
        ...
        try:
            ...
            if _validate_connection(target_connection) != source_version:
            ...
            target_digest = _logical_rows_digest(target_connection)
            ...
            if _serialized_digest(serialized, expected_version=expected_version) != target_digest:
        ...
        try:
            ...
            linked = True
            if not owned_regular(destination):
                raise ConfigurationError("SQLite backup publication identity unsafe")
            if (
                _serialized_digest(destination.read_bytes(), expected_version=expected_version)
                != target_digest
            ):
            ...
            temporary.unlink()
            linked = False
            if not private_regular(destination):
                raise ConfigurationError("SQLite backup final identity unsafe")
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                or published.st_uid != _effective_user_id()
