    ...
    def next_codex_lifecycle_job_id(self) -> UUID | None:
        ...
        with self.connection.transaction(), self.connection.cursor() as cursor:
            cursor.execute(
                ...
                " and jsonb_typeof(payload->'lifecycle_plan_body')='object')"
                " or ((select count(*) from jsonb_object_keys(payload))=3"
        ...
        *,
        ...
        with self.connection.transaction(), self.connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                " continuity_event.idempotency_key||':job:'||job.id::text"
                ...
                " and ((continuity_event.event_type in ('session_start','pre_close')"
                "   and (select count(*) from jsonb_object_keys(job.payload)) in (3,4)"
                ...
                "  or (continuity_event.event_type not in ('session_start','pre_close')"
                "   and (select count(*) from jsonb_object_keys(job.payload)) in (2,3)"
    ...
    def lookup_lifecycle_hydration(
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                ...
                " and event.event_type in ('session_start','pre_close')"
                " and ((event.event_type='session_start' and outbox.state='completed')"
                "   or (event.event_type='pre_close'"
                "     and outbox.state in ('pending','processing','completed')))"
        ...
        *,
        ...
        with self.connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                " and s.revision=(case when e.source_revision"
                " ~ '^git:[0-9a-f]{40};state:sha256:[0-9a-f]{64}$'"
                " then substring(e.source_revision from 5 for 40)"
                " else e.source_revision end) and s.tree_digest=%s"
        ...
        *,
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                ...
                " order by version desc limit 1) migration on true"
                " where job.realm_id=%s and job.id=%s"
                " and attempt.id=%s and ((job.state='recovery-required'"
                " and attempt.outcome='recovery-required') or (job.state='running'"
                " and attempt.outcome is null and exists(select 1"
                " from runtime.recovery_envelope_admission recovery_admission"
                " where recovery_admission.realm_id=job.realm_id"
                " and recovery_admission.old_job_id=job.id"
                " and recovery_admission.old_attempt_id=attempt.id"
                " and recovery_admission.old_claim_id=claim.id"
                " and recovery_admission.consumed_at is not null)))"
        ...
        *,
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                ...
                " outbox.id,coalesce(outbox.terminal_receipt_digest,"
                " lifecycle_admission.terminal_hook_receipt_digest),"
                " claim.id,claim.effect_digest,"
                " receipt.id,receipt.adapter_evidence_digest,plan_step.body,builder.risk"
                " from runtime.job job"
                " join runtime.job_attempt attempt on attempt.realm_id=job.realm_id"
                "  and attempt.job_id=job.id and attempt.id=%s"
                "  and ((%s and (attempt.outcome='recovery-required' or"
                "    (attempt.outcome is null and exists(select 1"
                "      from runtime.recovery_envelope_admission recovery_admission"
                "      where recovery_admission.realm_id=attempt.realm_id"
                "      and recovery_admission.old_job_id=attempt.job_id"
                "      and recovery_admission.old_attempt_id=attempt.id"
                "      and recovery_admission.consumed_at is not null)))"
                ...
                "   and context.source_revision=(case when plan.source_revision"
                "   ~ '^git:[0-9a-f]{40};state:sha256:[0-9a-f]{64}$'"
                "   then substring(plan.source_revision from 5 for 40)"
                "   else plan.source_revision end)"
                ...
                "   or (event.event_type<>'pre_close' and outbox.state='completed'"
                "    and outbox.terminal_receipt_digest is not null))"
                " left join client.codex_lifecycle_admission lifecycle_admission"
                " on lifecycle_admission.realm_id=event.realm_id"
                ...
                " where job.realm_id=%s and job.id=%s"
                " and ((%s and (job.state='recovery-required' or"
                "   (job.state='running' and exists(select 1"
                "     from runtime.recovery_envelope_admission recovery_admission"
                "     where recovery_admission.realm_id=job.realm_id"
                "     and recovery_admission.old_job_id=job.id"
                "     and recovery_admission.old_attempt_id=attempt.id"
                ...
                "   and lifecycle_admission.id is not null))"
                " and envelope.id=(select latest.id from runtime.execution_envelope latest"
