    ...
    def _terminal_runtime(
        ...
        with self.connection.cursor() as cursor:  # type: ignore[attr-defined]
            cursor.execute(
                ...
                " and (attempt.outcome='succeeded' or (attempt.outcome='recovery-required'"
                " and exists(select 1 from runtime.recovery_envelope_admission admission"
                ...
                " join runtime.job_attempt recovery_attempt"
                " on recovery_attempt.realm_id=admission.realm_id"
                " and recovery_attempt.id=admission.recovery_attempt_id"
                " and recovery_attempt.outcome='succeeded'"
                " join runtime.effect_claim recovery_claim"
                " on recovery_claim.realm_id=admission.realm_id"
                " and recovery_claim.id=admission.recovery_claim_id"
                " join runtime.effect_receipt recovery_receipt"
                " on recovery_receipt.realm_id=admission.realm_id"
                " and recovery_receipt.claim_id=recovery_claim.id"
                " and recovery_receipt.status='completed'"
                " where admission.realm_id=job.realm_id and admission.old_job_id=job.id"
                " and admission.old_attempt_id=attempt.id"
                " and exists(select 1 from runtime.effect_claim old_claim"
                " where old_claim.realm_id=admission.realm_id"
                " and old_claim.id=admission.old_claim_id and old_claim.job_id=job.id"
                " and old_claim.attempt_id=attempt.id)"
                " and admission.consumed_at is not null))) and attempt.finished_at is not null"
